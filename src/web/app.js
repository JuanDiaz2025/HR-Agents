...
async function viewReps() {
  const [{ reps }, summary] = await Promise.all([api('reps'), api('summary')]);
  ...
  return {
    html: `<h1>Reps</h1><p class="sub">Score per call, oldest first. Click a point to open that call.</p><div class="grid-2">${cards}</div>`,
    ...
    html: `
      ...
      <div class="card">
        ...
        <p class="small muted" style="margin:-6px 0 12px">Includes the evidence quotes and the whole transcript.
          <a href="/report/${esc(session.id)}" target="_blank" rel="noopener">Open in a new tab</a> to print or send it.</p>
